...
export async function redeemRechargeCardFixed(formData: FormData) {
  try {
    ...
    try {
      ...
      const { error: pointsError } = await adminClient.rpc('execute_raw_sql', {
        sql_query: `
          INSERT INTO points_transactions 
            (user_id, points, is_positive, description, created_by)
          VALUES 
            ('${data.user.id}', ${card.points}, true, E'استخدام بطاقة شحن', '${data.user.id}')
        `
      })
      ...
      await adminClient.rpc('execute_raw_sql', {
        sql_query: `
          ...
          VALUES
            ('${data.user.id}', E'شحن رصيد', E'تم شحن رصيدك بـ ${card.points} نقطة', false)
        `
      });
      ...
      await adminClient.rpc('execute_raw_sql', {
        sql_query: `
          ...
          VALUES
            ('${data.user.id}', 'redeem_card', E'شحن رصيد بكرت رقم ${cardCode} (${card.points} نقطة)')
        `
      });
