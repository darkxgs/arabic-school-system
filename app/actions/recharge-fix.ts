    ...
    try {
      ...
      if (directInsertError) {
        ...
        const sqlQuery = `
          ...
          VALUES 
            ('${data.user.id}', ${card.points}, true, 'استخدام بطاقة شحن', '${data.user.id}')
          RETURNING id
        `;
      ...
      try {
        ...
        try {
          const { data: calculationResult, error: calculationError } = await adminClient.rpc('execute_raw_sql', {
            sql_query: `
              WITH positive_points AS (
                SELECT COALESCE(SUM(points), 0) AS total
                FROM points_transactions
                WHERE user_id = '${data.user.id}' AND is_positive = true
              ),
              negative_points AS (
                SELECT COALESCE(SUM(points), 0) AS total
                FROM points_transactions
                WHERE user_id = '${data.user.id}' AND is_positive = false
              )
              SELECT 
