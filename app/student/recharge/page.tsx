    ...
    try {
      ...
      try {
        ...
        if (!error && data !== null && data > 0) {
          ...
          // Also update student_points table
          try {
            await supabase
              .from("student_points")
              .upsert({ 
            ...
            console.log("Updated student_points table with RPC value:", data)
          } catch (updateErr) {
            console.error("Failed to update student_points table:", updateErr)
          }
      ...
      try {
        ...
        if (!txError && transactions && transactions.length > 0) {
          ...
          if (total > 0) {
            ...
            // Update student_points table with calculated value
            try {
              await supabase
                .from("student_points")
                .upsert({ 
              ...
              console.log("Updated student_points table with calculated value:", total)
            } catch (updateErr) {
              console.error("Failed to update student_points table:", updateErr)
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Ensure the points are saved to student_points table
        if (result.points && userId) {
          try {
            console.log(`Updating student_points table with ${result.points} points`)
            ...
            if (updateError) {
              console.error("Failed to update student_points directly:", updateError)
            } else {
              console.log("Successfully updated student_points table")
            }
          } catch (updateErr) {
            console.error("Error updating student_points:", updateErr)
          }
        }
